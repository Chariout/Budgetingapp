...
import tn.esprit.budgetingapp.adapters.SectionExpenseAdapter;
import tn.esprit.budgetingapp.providers.ExpensesContract.Expenses;
import tn.esprit.budgetingapp.providers.ExpensesContract.ExpensesWithCategories;
import tn.esprit.budgetingapp.MainActivity;
import tn.esprit.budgetingapp.R;
import tn.esprit.budgetingapp.utils.Utils;
...
public class ReportFragment extends Fragment implements LoaderManager.LoaderCallbacks<Cursor>,
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        if (item.getItemId() == R.id.filter_expenses_menu_item) {
            View menuItemView = Objects.requireNonNull(getActivity()).findViewById(R.id.filter_expenses_menu_item);
    ...
    public boolean onMenuItemClick(MenuItem item) {
        ...
        if (itemId == R.id.today_filter_option) {
            makeTodaysReport();
            return true;
        } else if (itemId == R.id.week_filter_option) {
            makeWeeklyReport();
            return true;
        } else if (itemId == R.id.month_filter_option) {
            makeMonthlyReport();
            return true;
        } else if (itemId == R.id.date_filter_option) {
            makeDateReport();
            return true;
        } else if (itemId == R.id.range_filter_option) {
            makeDateRangeReport();
            return true;
        }
        return false;
    ...
    public androidx.loader.content.Loader<Cursor> onCreateLoader(int id, Bundle args) {
        int reportType = Objects.requireNonNull(args).getInt(REPORT_TYPE);
    ...
    private void makeTodaysReport() {
        ...
        Objects.requireNonNull(getActivity()).setTitle(getString(R.string.filter_todays_expenses));
    ...
    private void makeWeeklyReport() {
        ...
        Objects.requireNonNull(getActivity()).setTitle(getString(R.string.filter_weeks_expenses));
    ...
    private void makeMonthlyReport() {
        ...
        Objects.requireNonNull(getActivity()).setTitle(getString(R.string.filter_months_expenses));
    ...
    private void makeDateReport() {
        DatePickerDialog.OnDateSetListener listener = (view, year, month, day) -> {
            ...
            Objects.requireNonNull(getActivity()).setTitle(getString(R.string.filter_date_expenses, systemFormatDateStr));
        ...
        tn.esprit.budgetingapp.fragments.DatePickerFragment datePickerFragment = tn.esprit.budgetingapp.fragments.DatePickerFragment.newInstance(listener);
    ...
    private void makeDateRangeReport() {
        final DatePickerDialog.OnDateSetListener endDateListener = (view, year, month, day) -> {
            Calendar calendar = Calendar.getInstance();
            calendar.set(year, month, day);
            ...
            Objects.requireNonNull(getActivity()).setTitle(getString(R.string.filter_date_range_expenses,
        ...
        DatePickerDialog.OnDateSetListener startDateListener = (view, year, month, day) -> {
            ...
            datePickerFragment.show(Objects.requireNonNull(getActivity()).getSupportFragmentManager(), "end_date_picker");
        };

        tn.esprit.budgetingapp.fragments.DatePickerFragment datePickerFragment = tn.esprit.budgetingapp.fragments.DatePickerFragment.newInstance(startDateListener);
        datePickerFragment.show(Objects.requireNonNull(getActivity()).getSupportFragmentManager(), "start_date_picker");
    }
